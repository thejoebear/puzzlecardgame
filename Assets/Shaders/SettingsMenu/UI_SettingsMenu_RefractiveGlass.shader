Shader "UI/SettingsMenu/RefractiveGlass"
{
    Properties
    {
        ...
        [Header(Moving Background Stars)]
        _StarSpeed ("Star Speed (XY=Far, ZW=Near)", Vector) = (0.01, 0.005, 0.02, 0.01)
        _StarDensity ("Star Density (X=Far, Y=Near)", Vector) = (25.0, 12.0, 0.0, 0.0)
        _StarIntensity ("Star Intensity", Range(0.0, 5.0)) = 1.2
    ...
    SubShader
    {
        ...
        HLSLPROGRAM
            ...
            float3 getMovingStars(float2 uv, float density, float2 speed, float intensity, float twinkleSpeed) {
                float2 starUV = uv * density + _Time.y * speed;
                float2 gridID = floor(starUV);
                float2 gridUV = frac(starUV) - 0.5;
                
                float n = hash(gridID);
                
                // Random position offset within the cell
                float2 offset = float2(hash(gridID + 12.34), hash(gridID + 45.67)) - 0.5;
                float dist = length(gridUV - offset * 0.7);
                
                // Sharp star point
                float star = smoothstep(0.12, 0.0, dist);
                
                // Twinkle
                float twinkle = sin(_Time.y * twinkleSpeed + n * 6.28) * 0.5 + 0.5;
                
                // Star color variety
                float3 starColor = lerp(float3(0.7, 0.9, 1.0), float3(1.0, 0.95, 0.8), hash(gridID + 89.12));
                
                return star * twinkle * intensity * starColor;
            }
            ...
            float4 frag(v2f i) : SV_Target
            {
                ...
                // Moving Background Stars (Two layers for beautiful parallax)
                float3 starLayer1 = getMovingStars(i.texcoord, _StarDensity.x, _StarSpeed.xy, _StarIntensity * 0.4, _StarTwinkleSpeed);
                float3 starLayer2 = getMovingStars(i.texcoord, _StarDensity.y, _StarSpeed.zw, _StarIntensity * 0.8, _StarTwinkleSpeed * 1.5);
                float3 movingStars = starLayer1 + starLayer2;
