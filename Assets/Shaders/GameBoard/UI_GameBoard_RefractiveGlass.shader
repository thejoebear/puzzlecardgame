Shader "UI/GameBoard/RefractiveGlass"
{
    Properties
    {
        [PerRendererData] _MainTex ("Sprite Texture", 2D) = "white" {}
        _Color ("Tint", Color) = (0.1, 0.1, 0.15, 0.25)
        
        [Header(Crystal Details)]
        _ParallaxStrength ("Internal Parallax", Range(0, 0.1)) = 0.01
        _SparkleIntensity ("Sparkle Intensity", Range(0, 2.0)) = 0.1
        _InclusionIntensity ("Inclusion Intensity", Range(0, 2.0)) = 0.05
        _SparkleScale ("Sparkle Scale", Range(1, 50)) = 15.0
        _InclusionScale ("Inclusion Scale", Range(1, 50)) = 25.0
        _SparkleSharpness ("Sparkle Sharpness", Range(1, 100)) = 50.0
        _SparkleTwinkleSpeed ("Sparkle Twinkle Speed", Range(0, 5.0)) = 1.0
        
        [Header(Glass Effects)]
        _FresnelPower ("Fresnel Power", Range(0.1, 5.0)) = 3.0
        _FresnelIntensity ("Fresnel Intensity", Range(0, 2.0)) = 0.1
        _ChromaticAberration ("Chromatic Aberration", Range(0, 0.1)) = 0.0
        _UVDistortion ("UV Distortion Strength", Range(0, 0.3)) = 0.0
        _DistortionSpeed ("Distortion Speed", Range(0, 2.0)) = 0.2
        _RadialGradient ("Radial Gradient Intensity", Range(0, 1.0)) = 0.1
        _RadialFalloff ("Radial Falloff", Range(1, 10)) = 4.0
        
        [Header(Border Settings)]
        _BorderColor ("Border Color", Color) = (0.4, 0.7, 1.0, 0.4)
        _BorderWidth ("Border Width", Range(0, 0.5)) = 0.008
        _BorderGlow ("Border Glow", Float) = 0.8
        _EdgeGlowIntensity ("Edge Glow Intensity", Range(0, 0.5)) = 0.01
    ...
    SubShader
    {
        ...
        HLSLPROGRAM
            ...
            float3 getCrystalSparkles(float2 uv, float scale, float speed, float intensity, float sharpness) {
                ...
                // Simplified to single-octave noise for less visual noise and better performance
                float noise = smoothNoise(sUV + _Time.y * speed);
                
                // Create sharp crystal peaks
                float sparkle = pow(max(noise, 0.0), sharpness);
                
                // Smooth twinkling animation
                float twinkle = abs(sin(_Time.y * _SparkleTwinkleSpeed + sUV.x * 2.0 + sUV.y * 3.0));
            ...
            float4 sampleWithChromaticAberration(float2 uv) {
                // If Chromatic Aberration is zero/negligible, bypass for performance and perfect clarity
            ...
            float2 getUVDistortion(float2 uv) {
                if (_UVDistortion <= 0.0001) {
                    return uv;
                }
                // Simplified to single-octave noise
                float distortion = smoothNoise(uv * 4.0 + _Time.y * _DistortionSpeed) - 0.5;
            ...
            float4 frag(v2f i) : SV_Target
            {
                ...
                // Crystal Sparkles
                float3 sparkles = getCrystalSparkles(uvLayer1, _SparkleScale, 0.2, _SparkleIntensity, _SparkleSharpness);
                float3 inclusions = getCrystalSparkles(uvLayer2, _InclusionScale, 0.1, _InclusionIntensity, _SparkleSharpness * 0.6);
